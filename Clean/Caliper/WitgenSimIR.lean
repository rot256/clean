...
memory; this is the buffers-only `compile_space_le` and the static register metric
quoted as one number. -/
theorem compile_spaceBound {m : ℕ} {ir : WitgenIR (F p) m} {code : Stmt 64}
    (hcode : compile N ir = some code) :
    SpaceBound C (fun s => s.bufs 0 = envArr) code
      (fun s' => s'.bufs 1 = (Vector.map encF (ir.irEval env)).toArray)
      ((m : ℤ) + (code.regPeak₀ : ℤ)) := by
  refine ⟨(m : ℤ), (m : ℤ), ?_, le_refl _⟩
  intro s hbuf
  obtain ⟨s', t, d, pp, hex, hout⟩ := compile_sim p env N envArr henv hcode hbuf
  obtain ⟨hd, hpp⟩ := compile_space_le hcode hex
  exact ⟨s', t, d, pp, hex, hout, hd, hpp⟩
...
theorem isZero_witgen_spaceBound {env : ProverEnvironment (F pBabybear)}
  ...
  intro s hbuf
  obtain ⟨s', d, pp, hex, hout, hpp⟩ := isZero_witgen_correct_139 henv hN0 hN hbuf
  have hd := (compile_space_le hcode hex).1
  exact ⟨s', 139, d, pp, hex, hout, by simpa using hd, hpp⟩
