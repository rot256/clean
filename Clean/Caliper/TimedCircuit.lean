...
A `TimedCircuit` is a `FormalCircuit` that also certifies its witness-generation
cost: constructing one requires, for every witness generator of the circuit,
compiled machine code (via `compile`) whose static cost sums below `witgenBudget`.

The obligation is one boolean evaluation (`underBudget`), dischargeable by
`native_decide`. What the number means:

* `FlatOperation.witgenTime` folds `FlatOperation.witgenCost` over the circuit's flat
  operations, threading the offset like `FlatOperation.localLength` and
  `computableChecks`, so each generator is priced, and its `envBound` checked, at its
  own accumulated offset.
* `witgenTime_sound` extracts from `witgenTime = some T` a per-generator certificate
  (`WitnessCosts`): the compiler accepts every generator at its own offset with a
  static time `tᵢ`, and `Σ tᵢ = T`.
* By phase 2 each `tᵢ` is the exact running time of every execution of that compiled
  generator, with live-memory peak `≤ tᵢ` in any per-word-charging model; by
  `compile_sim` that execution computes the encoded reference output.

The type is unattainable for circuits with a bare `.native` witness closure, since
`compile` rejects those and `witgenTime` returns `none`. A `.certified` witness
restores it, the cost certificate then applying to the closure the prover runs.

Input-genericity is a design choice. A compiled generator's cost depends on the
syntactic shape of the expressions it embeds, and a circuit's input variable is an
arbitrary expression tree, so no single number is generic over all instantiations.
The obligation is stated at the canonical instantiation: fresh input variables
`varFromOffset Input 0` with witnesses from offset `size Input`. A structural
...
over composition by construction, but the composed machine-level run is not yet
stated.
-/
...
checked compiler accepts the generator at environment size `n`, priced by the partial
...
/-- Whenever the fold produces `some T`, every witness
...
/-- The obligation restated: the canonical instantiation's total certified witgen
...
/-- The existing `FormalCircuit` upgraded to a `TimedCircuit`
...
/-- End to end for the demo: every witness generator of
...
/-- The `< 2^40` form for the demo: the certified per-generator times `t ≤ 157`
