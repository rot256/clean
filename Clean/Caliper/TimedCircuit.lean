...
The circuit's two witness generators — `testIsZero` (the inverse witness, 154 unit
steps including its 15 register acquisitions) and `isZeroCircuitCopyIR` (the `<==`
copy, 31 unit steps including its 13 acquisitions) — sum to 185, far below the
default `2^40` budget. The obligation is one `native_decide`. -/

/- The pinned total for the instantiated `IsZeroField` circuit: `154 + 31 = 185`
...
/-- The timed circuit's certified total, pinned: 185 unit steps. -/
...
exactly its certified time `t ≤ 185`, with live-memory peak `≤ t`. -/
theorem isZeroTimed_witgen_le_185 :
    FlatOperation.forAll (size field)
      { witness n _ ir := ∃ t code, compile n ir = some code ∧
          code.staticTime? CostModel.unit = some t ∧ t ≤ 185 ∧
          ∀ {s s' : State 64} {t' : ℕ} {d p : ℤ},
...
/-- The `< 2^40` reading for the demo: the certified per-generator times `t ≤ 185`
of `isZeroTimed_witgen_le_185` are in particular strictly below the `2^40`
budget. -/
theorem isZeroTimed_witgen_lt_2_40 :
  ...
  refine forAll_witness_mono ?_ isZeroTimed_witgen_le_185
  rintro n m ir ⟨t, code, hc, ht, hle, hexec⟩
  exact ⟨t, code, hc, ht, by omega, hexec⟩
