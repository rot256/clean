...
these wrappers turn that into `montVal`, the form the compiler induction carries. The
lowering always keeps the modulus at register `0` and the Montgomery constant at
`k + 1`, so the wrappers fix those and read them off one `PreludeEnc`. -/
...
theorem cast_two_pow_mod (n : ℕ) : ((2 ^ n % p : ℕ) : F p) = (2 : F p) ^ n := by
  rw [ZMod.natCast_mod]; push_cast; ring

/-- **Addition of Montgomery forms.** -/
theorem montAdd_field {k a b w : ℕ} {s : State 64} {pv : ℕ} {x y : F p}
    (hpR : p < 2 ^ (64 * k)) (hpre : PreludeEnc p pv k s)
    (hw : k + 3 ≤ w) (haw : a + k ≤ w) (hbw : b + k ≤ w)
    (har : RegsEnc s a k (montVal k x)) (hbr : RegsEnc s b k (montVal k y)) :
    ∃ s' tt dd pp, Exec C (montAdd k a b 0 w) s s' tt dd pp ∧
      RegsEnc s' (montAddOut k w) k (montVal k (x + y)) ∧
      (∀ q, q < w → s'.regs q = s.regs q) ∧
      s'.bufs = s.bufs ∧ s'.caps = s.caps := by
  have hp : 0 < p := (Fact.out (p := p.Prime)).pos
  obtain ⟨s', tt, dd, pp, hex, hout, hpres, hbuf, hcap⟩ :=
    montAdd_exec (C := C) ⟨haw, hbw, by omega⟩ hp hpR
      (montVal_lt k x) (montVal_lt k y)
      (fun j hj => hpre.modulus j (by omega)) har hbr
...
theorem montMul_field {k a b w : ℕ} {s : State 64} {pv : ℕ} {x y : F p}
    (hk : 0 < k) (hp2 : 2 < p) (hpR : p < 2 ^ (64 * k))
    (hpinv : (p * pv + 1) % 2 ^ 64 = 0) (hpre : PreludeEnc p pv k s)
    (hw : k + 3 ≤ w) (haw : a + k ≤ w) (hbw : b + k ≤ w)
    (har : RegsEnc s a k (montVal k x)) (hbr : RegsEnc s b k (montVal k y)) :
    ∃ s' tt dd pp, Exec C (montMulSOS k a b 0 (k + 1) w) s s' tt dd pp ∧
      RegsEnc s' (montOut k w) k (montVal k (x * y)) ∧
      (∀ q, q < w → s'.regs q = s.regs q) ∧
      s'.bufs = s.bufs ∧ s'.caps = s.caps := by
  obtain ⟨k', rfl⟩ : ∃ k', k = k' + 1 := ⟨k - 1, by omega⟩
  have hp : 0 < p := (Fact.out (p := p.Prime)).pos
  have hAB : montVal (k' + 1) x * montVal (k' + 1) y < p * 2 ^ (64 * (k' + 1)) :=
    Nat.mul_lt_mul'' (montVal_lt (k' + 1) x) (lt_trans (montVal_lt (k' + 1) y) hpR)
  obtain ⟨s', tt, dd, pp, hex, ⟨V, hV, hVlt, hVmod⟩, hpres, hbuf, hcap⟩ :=
    montMulSOS_exec (C := C) ⟨haw, hbw, by omega, by omega⟩ hp hpR hpinv
      (lt_trans (montVal_lt (k' + 1) x) hpR) (lt_trans (montVal_lt (k' + 1) y) hpR)
      hAB hpre.modulus hpre.const har hbr
  refine ⟨s', tt, dd, pp, hex, ?_, hpres, hbuf, hcap⟩
...
theorem montMulConst_field {k c a w A : ℕ} {s : State 64} {pv : ℕ}
    (hk : 0 < k) (hpR : p < 2 ^ (64 * k)) (hpinv : (p * pv + 1) % 2 ^ 64 = 0)
    (hpre : PreludeEnc p pv k s) (hw : k + 3 ≤ w) (haw : a + k ≤ w)
    (hA : A < 2 ^ (64 * k)) (hc : c < p) (har : RegsEnc s a k A) :
    ∃ s' tt dd pp V, Exec C (montMulConst k c a 0 (k + 1) w) s s' tt dd pp ∧
      V < p ∧ (V : F p) * (2 : F p) ^ (64 * k) = (A : F p) * (c : F p) ∧
      RegsEnc s' (montMulConstOut k w) k V ∧
      (∀ q, q < w → s'.regs q = s.regs q) ∧
      s'.bufs = s.bufs ∧ s'.caps = s.caps := by
  obtain ⟨k', rfl⟩ : ∃ k', k = k' + 1 := ⟨k - 1, by omega⟩
  have hp : 0 < p := (Fact.out (p := p.Prime)).pos
  obtain ⟨s', tt, dd, pp, hex, ⟨V, hV, hVlt, hVmod⟩, hpres, hbuf, hcap⟩ :=
    montMulConst_exec (C := C) ⟨haw, by omega, by omega⟩ hp hpR hpinv hA hc
...
theorem leaveMont_field {k a w : ℕ} {s : State 64} {pv : ℕ} {x : F p}
    ...
    ∃ s' tt dd pp, Exec C (leaveMont k a w) s s' tt dd pp ∧
      RegsEnc s' (montMulConstOut k w) k x.val ∧
      (∀ q, q < w → s'.regs q = s.regs q) ∧
      s'.bufs = s.bufs ∧ s'.caps = s.caps := by
  obtain ⟨s', tt, dd, pp, V, hex, hVlt, hVeq, hV, hpres, hbuf, hcap⟩ :=
    montMulConst_field (C := C) (c := 1) hk hpR hpinv hpre hw haw
      (lt_trans (montVal_lt k x) hpR) (by omega) har
  refine ⟨s', tt, dd, pp, hex, ?_, hpres, hbuf, hcap⟩
  have hval : V = x.val := by
    refine eq_of_natCast hVlt (ZMod.val_lt x) ?_
    rw [ZMod.natCast_val, ZMod.cast_id]
    rw [montVal_cast, Nat.cast_one, mul_one] at hVeq
    exact mul_right_cancel₀ (two_pow_ne_zero hp2 (64 * k)) hVeq
  rwa [hval] at hV
...
theorem enterMont_field {k a w A : ℕ} {s : State 64} {pv : ℕ} {x : F p}
    ...
    (hpinv : (p * pv + 1) % 2 ^ 64 = 0) (hpre : PreludeEnc p pv k s)
    (hw : k + 3 ≤ w) (haw : a + k ≤ w)
    (hA : A < 2 ^ (64 * k)) (hAx : (A : F p) = x) (har : RegsEnc s a k A) :
    ∃ s' tt dd pp,
      Exec C (enterMont k (2 ^ (2 * (64 * k)) % p) a w) s s' tt dd pp ∧
      RegsEnc s' (montMulConstOut k w) k (montVal k x) ∧
      (∀ q, q < w → s'.regs q = s.regs q) ∧
      s'.bufs = s.bufs ∧ s'.caps = s.caps := by
  have hp : 0 < p := (Fact.out (p := p.Prime)).pos
  obtain ⟨s', tt, dd, pp, V, hex, hVlt, hVeq, hV, hpres, hbuf, hcap⟩ :=
    montMulConst_field (C := C) (c := 2 ^ (2 * (64 * k)) % p) hk hpR hpinv hpre hw haw
      hA (Nat.mod_lt _ hp) har
  refine ⟨s', tt, dd, pp, hex, ?_, hpres, hbuf, hcap⟩
  have hval : V = montVal k x := by
    refine eq_montVal hVlt ?_
    rw [cast_two_pow_mod, hAx] at hVeq
    refine mul_right_cancel₀ (two_pow_ne_zero hp2 (64 * k)) (hVeq.trans ?_)
    rw [show 2 * (64 * k) = 64 * k + 64 * k by ring, pow_add]
    ring
  rwa [hval] at hV
...
theorem montInv_field {k a w : ℕ} {s : State 64} {pv : ℕ} {x : F p}
    ...
    ∃ s' tt dd pp,
      Exec C (invLimbs k a 0 w ;;
        montMulConst k (2 ^ (3 * (64 * k)) % p) (invOut k w) 0 (k + 1)
          (w + 5 * k + 16)) s s' tt dd pp ∧
      RegsEnc s' (montMulConstOut k (w + 5 * k + 16)) k (montVal k x⁻¹) ∧
      (∀ q, q < w → s'.regs q = s.regs q) ∧
      s'.bufs = s.bufs ∧ s'.caps = s.caps := by
  have hp : 0 < p := (Fact.out (p := p.Prime)).pos
  have hRf : (2 : F p) ^ (64 * k) ≠ 0 := two_pow_ne_zero hp2 _
  obtain ⟨s₁, t₁, d₁, q₁, hex₁, ⟨S, hS, hSlt, hSzero, hSinv⟩, hpres₁, hbuf₁, hcap₁⟩ :=
    invLimbs_exec (C := C) (show 0 + k ≤ w by omega) haw hkb
      (odd_of_two_lt hp2) hpR (by omega) (montVal_lt k x) har
  ...
  obtain ⟨s', tt, dd, pp, V, hex, hVlt, hVeq, hV, hpres, hbuf, hcap⟩ :=
    montMulConst_field (C := C) (c := 2 ^ (3 * (64 * k)) % p) hk hpR hpinv hpre₁
      (show k + 3 ≤ w + 5 * k + 16 by omega)
      (show invOut k w + k ≤ w + 5 * k + 16 by simp only [invOut]; omega)
      (lt_trans hSlt hpR) (Nat.mod_lt _ hp) hS
  refine ⟨s', _, _, _, .seq hex₁ hex, ?_, ?_, hbuf.trans hbuf₁, hcap.trans hcap₁⟩
  · have hval : V = montVal k x⁻¹ := by
      refine eq_montVal hVlt ?_
      rw [cast_two_pow_mod] at hVeq
      rw [show 3 * (64 * k) = 64 * k + (64 * k + 64 * k) by ring, pow_add, pow_add]
        at hVeq
      refine mul_right_cancel₀ hRf (hVeq.trans ?_)
      by_cases hx : x = 0
      · subst hx
        have hA0 : montVal k (0 : F p) = 0 := by simp [montVal, ZMod.val_zero]
        rw [hSzero hA0]
        simp
      · have hcop : Nat.gcd (montVal k x) p = 1 := by
          have hA0 : montVal k x ≠ 0 := by
            intro h
            have hcx := montVal_cast k x
            rw [h] at hcx
            refine hx ?_
            have h0 : (0 : F p) = x * (2 : F p) ^ (64 * k) := by simpa using hcx
            exact (mul_eq_zero.mp h0.symm).resolve_right hRf
          have hnd : ¬ p ∣ montVal k x := fun hd =>
            absurd (Nat.le_of_dvd (Nat.pos_of_ne_zero hA0) hd)
              (by have := montVal_lt k x; omega)
        ...
        calc (S : F p) * ((2 : F p) ^ (64 * k)
                * ((2 : F p) ^ (64 * k) * (2 : F p) ^ (64 * k))) * x
            = ((S : F p) * (x * (2 : F p) ^ (64 * k)))
              * ((2 : F p) ^ (64 * k) * (2 : F p) ^ (64 * k)) := by ring
          _ = (2 : F p) ^ (64 * k) * (2 : F p) ^ (64 * k) := by rw [hSA, one_mul]
          _ = x⁻¹ * (2 : F p) ^ (64 * k) * (2 : F p) ^ (64 * k) * x := by
              rw [show x⁻¹ * (2 : F p) ^ (64 * k) * (2 : F p) ^ (64 * k) * x
                  = (x⁻¹ * x) * ((2 : F p) ^ (64 * k) * (2 : F p) ^ (64 * k)) by ring,
