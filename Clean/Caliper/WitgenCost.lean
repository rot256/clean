...
theorem compile_total_footprint_le {N m : ℕ} {ir : WitgenIR F m} {code : Stmt 64}
    ...
    p + (code.regPeak₀ : ℤ) ≤ ((code.liveBefore ∅).card + t : ℤ) :=
  hx.straight_total_footprint_le (compile_straight hc)
...
theorem isZeroCompiled_staticTime_cycles : isZeroCompiled.staticTime .cycles = 2040 := by
  native_decide
...
/- Nothing is read before being written: the compiled code is closed, taking its
input from the environment buffer rather than from an incoming register. -/
/-- info: [] -/
...
/- The certified program's pinned cost numerals: the same 139 unit steps / 2040
